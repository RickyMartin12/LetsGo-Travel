...
class Merlin {
	...
	public function admin_page() {
		...
		$suffix = ( ( true == $this->dev_mode ) ) ? '' : ''; // $suffix = ( ( true == $this->dev_mode ) ) ? '' : '.min';
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'trawell' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'trawell' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'trawell' );
		}
	...
	function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'trawell' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'trawell' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'trawell' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'trawell' ),
		...
		<footer class="merlin__content__footer">
			...
			<?php if ( ! $is_child_theme ) : ?>
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next button-next" data-callback="install_child">
					<span class="merlin__button--loading__text"><?php echo esc_html( $install ); ?></span><?php echo wp_kses_post( $this->loading_spinner() ); ?>
				</a>
					...
					<li data-slug="<?php echo esc_attr( $slug ); ?>">
						...
						<span>
							...
							if ( isset( $plugins['install'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Install', 'trawell' );
							}
							if ( isset( $plugins['update'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Update', 'trawell' );
							}
							if ( isset( $plugins['activate'][ $slug ] ) ) {
								$keys[] = esc_html__( 'Activate', 'trawell' );
							}
							echo implode( esc_html__( 'and', 'trawell' ) , $keys );
							?>
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				<p><?php esc_html_e( 'Select which demo data you want to import:', 'trawell' ); ?></p><br>
				<p><select class="js-merlin-demo-import-select">
		...
		<form action="" method="post">
			...
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php echo merlin_print_import_steps( $this->get_import_steps_html( $import_info ) ); ?>
			</ul>
			...
			<footer class="merlin__content__footer">
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="merlin__button merlin__button--next button-next" data-callback="install_content">
					<span class="merlin__button--loading__text"><?php echo esc_html( $import ); ?></span><?php echo wp_kses_post( $this->loading_spinner() ); ?>
				</a>
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'trawell' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'trawell' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'trawell' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'trawell' ) ) );
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Invalid content!', 'trawell' ) ) );
		}
        ...
        if ( $json ) {
            ...
            wp_send_json( array(
                'error'   => 1,
                'message' => esc_html__( 'Error', 'trawell' ),
                'logs'    => '',
                'errors'  => '',
            ) );
        }
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				...
				'description'       => esc_html__( 'Demo content data.', 'trawell' ),
				'pending'           => esc_html__( 'Pending', 'trawell' ),
				'installing'        => esc_html__( 'Installing', 'trawell' ),
				'success'           => esc_html__( 'Success', 'trawell' ),
		...
		if ( ! empty( $import_files['widgets'] )  ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'trawell' ),
				'description'      => esc_html__( 'Sample widgets data.', 'trawell' ),
				'pending'          => esc_html__( 'Pending', 'trawell' ),
				'installing'       => esc_html__( 'Installing', 'trawell' ),
				'success'          => esc_html__( 'Success', 'trawell' ),
		...
		if ( ! empty( $import_files['sliders'] )  ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'trawell' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'trawell' ),
				'pending'          => esc_html__( 'Pending', 'trawell' ),
				'installing'       => esc_html__( 'Installing', 'trawell' ),
				'success'          => esc_html__( 'Success', 'trawell' ),
		...
		if ( ! empty( $import_files['options'] )  ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'trawell' ),
				'description'      => esc_html__( 'Sample theme options data.', 'trawell' ),
				'pending'          => esc_html__( 'Pending', 'trawell' ),
				'installing'       => esc_html__( 'Installing', 'trawell' ),
				'success'          => esc_html__( 'Success', 'trawell' ),
		...
		if ( ! empty( $import_files['redux'] )  ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'trawell' ),
				'description'      => esc_html__( 'Redux framework options.', 'trawell' ),
				'pending'          => esc_html__( 'Pending', 'trawell' ),
				'installing'       => esc_html__( 'Installing', 'trawell' ),
				'success'          => esc_html__( 'Success', 'trawell' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'trawell' ),
				'description'      => esc_html__( 'After import setup.', 'trawell' ),
				'pending'          => esc_html__( 'Pending', 'trawell' ),
				'installing'       => esc_html__( 'Installing', 'trawell' ),
				'success'          => esc_html__( 'Success', 'trawell' ),
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'trawell' );
		$data['proceed']  = 'true';
...
function merlin_print_import_steps($html){
	return $html;
}